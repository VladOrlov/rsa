...
BigInteger::~BigInteger() { delete[] number; }
...
vector<char> BigInteger::toBinary() {
...
int BigInteger::size() { return currentSize; }
...
//a^d(mod m) 
BigInteger BigInteger::calcPowMod(const vector<char> &d, const BigInteger m) {
    BigInteger res;
    res = *this;
...
BigInteger BigInteger::getMaxDivider(BigInteger b) {
    BigInteger a(*this);
    BigInteger c;
    while (b != 0) {
        c = a % b;
...
BigInteger BigInteger::operator+(const BigInteger &obj) {
    ...
    if ((positive && obj.positive) || (!positive && !obj.positive)) {
        ...
        if (!positive && !obj.positive)
            temp.positive = false;
    } else if ((!positive && obj.positive) || (positive && !obj.positive)) {
        if (!isAbsBigger(obj)) {
...
BigInteger BigInteger::operator%(const BigInteger &obj) {
    if (obj.currentSize == 0)
        throw MathException("remainder of division by zero");
    ...
    while (i < obj.maxSize) {
        ...
        if (bf == '-') {
            ...
            if (bf == '0' || bf == '1' || bf == '2' || bf == '3' || bf == '4' ||
                ...
                throw MathException("Incorrect input data format!");
            }
        }
    }
...
void BigInteger::random(unsigned int length) {
    if (length <= maxSize) {
        ...
        throw MathException("Specified size exceeds the maximum capacity of the number");
    }
}
